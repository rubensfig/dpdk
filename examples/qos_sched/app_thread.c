...
update_backpressure_state(uint16_t port)
{
	...
	uint64_t check_interval = stats->tsc_hz / 100;
	...
	if (!stats->backpressure_active) {
		...
		if (total_tx > 1000) { /* Minimum sample size */
			double drop_rate = ((double)total_drops / (double)total_tx)*100000;
			if (drop_rate > 0.05) {
		...
		if (total_tx > 1000) {
			double drop_rate = ((double)total_drops / (double)total_tx)*1000;
...
app_tx_thread(struct thread_conf **confs)
{
	...
	while ((conf = confs[conf_idx])) {
		...
		if (likely(nb_pkts != 0)) {
			...
			// }
       			if (likely(nb_tx > 0)) {
				__atomic_add_fetch(&port_statistics[0].tx, nb_tx, __ATOMIC_RELAXED);
			}
		}
