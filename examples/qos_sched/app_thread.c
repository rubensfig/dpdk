...
app_rx_thread(struct thread_conf **confs)
{
	...
	while ((conf = confs[conf_idx])) {
		...
		// fprintf(stderr, "Port %u: rx=%d, ring_enqueue...\n", conf->rx_port, conf_idx);
    ...
    while ((conf = confs[conf_idx])) {
        ...
        if (!has_pending) {
            ...
            nb_pkt_deq = rte_sched_port_dequeue(conf->sched_port,
                                                mbufs,
                                                burst_conf.qos_dequeue, NULL);
                                                // burst_conf.qos_dequeue, port_statistics[conf->tx_port].subport_capacity);
		...
		if (nb_tx < nb_pkt_deq) {
		    uint16_t unsent = nb_pkt_deq - nb_tx;
    		    rte_memcpy(&pending_mbufs[pending_cnt],        // destination: append to pending
               			&mbufs[nb_tx],                     // source: unsent mbufs
               			unsent * sizeof(struct rte_mbuf*));// number of bytes to copy
    		    pending_cnt += unsent;
		    has_pending = true;

		} else {
		    APP_STATS_ADD(conf->stat.nb_tx, 1);
		}
	    	}
        } else {
